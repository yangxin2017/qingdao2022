<template>
  <div class="app-container">
    ...
    <div class="right-container  animated" :class="{'fadeOutRight': !showPanel, 'fadeInRight': showPanel}">
      ...
      <sq-card height="18%" class="sep" :title="'基本情况'">
        <div class="fk-lists">
          <sq-v-icon-text class="fk" v-for="item in ydDatas2" :key="item.id" :title="item.title" :data="item.data"></sq-v-icon-text>
        </div>
      </sq-card>
...
export default {
  ...
  data() {
    return {
      ...
      ydDatas: [
        { id: 1, title: '人口数', data: { type: 'text', val: '980', dw: '万' } },
        { id: 2, title: '职住平衡指数', data: { type: 'text', val: 74, dw: '%' } },
        { id: 3, title: '职住分离度', data: { type: 'text', val: 33, dw: '%' } },
      ],

      ydDatas2: [
        { id: 1, title: '单程平均通勤耗时', data: { type: 'text', val: '980', dw: '分钟' } },
        { id: 2, title: '45min以内通勤人口比重', data: { type: 'text', val: 74, dw: '%' } },
        { id: 3, title: '居民出行延误比', data: { type: 'text', val: 33, dw: '%' } },
      ],
